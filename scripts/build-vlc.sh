...
if [ ! -f ${BUILD_FOLDER}Makefile ] || [ ${BUILD_FOLDER}Makefile -ot ${SRC_FOLDER}configure.ac ] ; then
    ...
    ac_cv_func_strnlen=yes ac_cv_have_decl_strnlen=yes ac_cv_func_timespec_get=$HAS_TIMESPEC ac_cv_func_if_nametoindex=no ac_cv_c_restrict=restrict ac_cv_c_compiler_gnu=no ac_cv_host=$BUILD_HOST \
