...
@set CMAKE_VS=MSYS Makefiles
REM ~ @set CMAKE_VS=Visual Studio 14 2015
REM ~ @IF "%CMAKE_SYSTEM_PROCESSOR%"=="ARM" set CMAKE_VS=%CMAKE_VS% ARM
REM ~ @IF "%CMAKE_SYSTEM_PROCESSOR%"=="amd64" set CMAKE_VS=%CMAKE_VS% Win64
...
:call_main
%WD%%MSYSCON% --hold always /usr/bin/bash scripts/main-llvm.sh %*
